(function () {
        ...
        },
        {
          ...
          right: {
            ...
            rows: [
              ...
              { label: 'Income tax subtotal', value: '$6,120' },
  ...
  function seedStars() {
    ...
    for (var i = 0; i < 60; i++) {
      ...
      var top = (Math.random() * 68).toFixed(1);
      var delay = (Math.random() * 4).toFixed(2);
      var size = (Math.random() * 1.6 + 1).toFixed(1);
      html += '<span class="star" style="left:' + left + '%;top:' + top + '%;width:' + size + 'px;height:' + size + 'px;animation-delay:' + delay + 's;"></span>';
    }
    el.innerHTML = html;
  }

  function seedMotes() {
    var el = document.querySelector('.motes');
    if (!el) return;
    var html = '';
    for (var i = 0; i < 18; i++) {
      var left = (Math.random() * 100).toFixed(1);
      var duration = (Math.random() * 8 + 11).toFixed(1);
      var delay = (Math.random() * -19).toFixed(1);
      html += '<span class="mote" style="left:' + left + '%;animation-duration:' + duration + 's;animation-delay:' + delay + 's;"></span>';
    }
    el.innerHTML = html;
  }

  // ---------- Shelf hotspots + fallback list ----------

  var hotspotsEl = document.getElementById('shelf-hotspots');
  var fallbackListEl = document.getElementById('book-fallback-list');

  function renderShelf() {
    hotspotsEl.innerHTML = BOOKS.map(function (b) {
      return '' +
        '<button type="button" class="spine ' + b.spineClass + '" style="--x:' + b.hotspot.x + ';--y:' + b.hotspot.y + '" data-id="' + b.id + '" aria-label="Open Book ' + b.number + ' — ' + b.title + '">' +
          '<span class="spine-label">' + b.number + ' · ' + b.title + '</span>' +
        '</button>';
    }).join('');

    fallbackListEl.innerHTML = BOOKS.map(function (b) {
      return '' +
        '<li><button type="button" data-id="' + b.id + '">' +
          '<span class="fb-title">' + b.number + ' · ' + b.title + '</span>' +
          '<span class="fb-tag">' + b.tagline + '</span>' +
        '</button></li>';
    }).join('');

    var openers = document.querySelectorAll('.spine, .book-fallback-list button');
  ...
  var readerEyebrow = document.getElementById('reader-eyebrow');
  var readerTitle = document.getElementById('reader-title');
  var chapterTabsEl = document.getElementById('chapter-tabs');
  var pageLeftEl = document.getElementById('page-left');
  var pageRightEl = document.getElementById('page-right');
  var readerBack = document.getElementById('reader-back');
  var readerForward = document.getElementById('reader-forward');
  var readerPosition = document.getElementById('reader-position');
  var readerShelve = document.getElementById('reader-shelve');
  var backdrop = document.getElementById('book-backdrop');
  ...
  function renderTabs() {
    chapterTabsEl.innerHTML = currentBook.spreads.map(function (s, i) {
      return '<button type="button" class="chapter-tab' + (i === currentSpreadIndex ? ' active' : '') + '" data-index="' + i + '">' + s.chapter + '</button>';
    }).join('');
    Array.prototype.forEach.call(chapterTabsEl.querySelectorAll('.chapter-tab'), function (el) {
      el.addEventListener('click', function () {
        currentSpreadIndex = Number(el.getAttribute('data-index'));
        renderSpread();
      });
    });
  }
  ...
  function renderSpread() {
    var spread = currentBook.spreads[currentSpreadIndex];
    ...
    Array.prototype.forEach.call(chapterTabsEl.querySelectorAll('.chapter-tab'), function (el, i) {
      el.classList.toggle('active', i === currentSpreadIndex);
    });

    readerBack.disabled = currentSpreadIndex === 0;
    readerForward.disabled = currentSpreadIndex === currentBook.spreads.length - 1;
    readerPosition.textContent = spread.chapter + ' · ' + (currentSpreadIndex + 1) + ' of ' + currentBook.spreads.length;
  }

  spreadEl.addEventListener('click', function (e) {
    var btn = e.target.closest ? e.target.closest('.option-btn') : null;
    if (!btn || btn.disabled) return;

    var side = btn.getAttribute('data-side');
    var idx = Number(btn.getAttribute('data-index'));
    var block = currentBook.spreads[currentSpreadIndex][side];
    var isCorrect = idx === block.correct;
    var container = btn.closest('.question-block');
    ...
    explanation.textContent = (isCorrect ? 'Correct — ' : 'Not quite — ') + block.explanation;
  });

  readerBack.addEventListener('click', function () {
    if (currentSpreadIndex > 0) { currentSpreadIndex--; renderSpread(); }
  });
  readerForward.addEventListener('click', function () {
    if (currentSpreadIndex < currentBook.spreads.length - 1) { currentSpreadIndex++; renderSpread(); }
  });
  readerShelve.addEventListener('click', closeBook);
  backdrop.addEventListener('click', closeBook);
  document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape' && overlay.classList.contains('active')) closeBook();
  });

  // ---------- Subtle pointer tilt (stat cards) ----------
  ...
  function initTilt() {
    ...
    Array.prototype.forEach.call(els, function (el) {
      ...
      el.addEventListener('mousemove', function (e) {
        ...
        el.style.transform = 'perspective(700px) rotateX(' + (-py * 6).toFixed(2) + 'deg) rotateY(' + (px * 8).toFixed(2) + 'deg) translateZ(0)';
      });
